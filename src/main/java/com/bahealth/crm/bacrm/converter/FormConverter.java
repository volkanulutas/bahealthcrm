...
public class FormConverter {
    ...
    public static Form toEntity(FormDto source) {
        ...
        Set<FormMessage> incomingFormMessageList = source.getIncomingMessageList().stream().map(FormMessageConverter::toEntity).collect(Collectors.toSet());
        target.setIncomingMessages(incomingFormMessageList);
        Set<FormMessage> outgoingFormMessageList = source.getOngoingMessageList().stream().map(FormMessageConverter::toEntity).collect(Collectors.toSet());
    ...
    public static FormDto toDto(Form source) {
        ...
        Set<FormMessageDto> incomingFormMessageList = source.getIncomingMessages().stream().map(FormMessageConverter::toDto).collect(Collectors.toSet());
        target.setIncomingMessageList(incomingFormMessageList);
        Set<FormMessageDto> outgoingFormMessageList = source.getOngoingMessages().stream().map(FormMessageConverter::toDto).collect(Collectors.toSet());
